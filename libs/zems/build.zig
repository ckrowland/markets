...
pub const EmscriptenStep = struct {
    ...
    pub fn init(b: *std.Build.Builder) *@This() {
        ...
        const r = b.allocator.create(EmscriptenStep) catch unreachable;
        r.* = .{
            .b = b,
    ...
    pub fn link(self: *@This(), exe: *std.Build.CompileStep) void {
        ...
        if (self.args.exported_functions.items.len > 0) {
            const s = std.mem.join(self.b.allocator, "','", self.args.exported_functions.items) catch unreachable;
            const ss = std.fmt.allocPrint(self.b.allocator, "-sEXPORTED_FUNCTIONS=['{s}']", .{s}) catch unreachable;
            emlink.addArg(ss);
        }
...
pub const EmscriptenArgs = struct {
    ...
    pub fn setDefault(self: *@This(), build_mode: std.builtin.Mode, max_optimizations: bool) void {
        ...
        self.other_args.append("-sUSE_OFFSET_CONVERTER") catch unreachable;
        switch (build_mode) {
            .Debug => {
    ...
    pub fn setOrAssertOption(self: *@This(), key: []const u8, value: []const u8) void {
        const v = self.options.getOrPut(key) catch unreachable;
        if (v.found_existing) {
