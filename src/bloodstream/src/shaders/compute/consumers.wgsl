...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    consumers[index].color = vec4(1, 0, 0, 0);
    ...
    for (var i = 0u; i < num_points; i += 1u) {
        ...
        for (var j = 0.0; j <= num_spline_balls; j += 1.0) {
            ...
            let dist = length(n);
            if (dist < radius) {
                //let overlap = src_ball.radius + radius - dist;
            ...
            //If next step we pass through the ball, collide.
            //Create ray from ball position to next position
            let pos = src_ball.position.xy;
            let next_pos = src_ball.position.xy + src_ball.velocity.xy;
            let slope = (next_pos.y - pos.y) / (next_pos.x - pos.x);
            let b = pos.y / (slope * pos.x);

            //Check if spline ball is on line
            let dist_from_line = center_pos.y - ((slope * center_pos.x) + b);
            let on_line = dist_from_line < 0.5;
            let line_near = distance(pos, center_pos) < distance(pos, next_pos);
            let fake_n = vec2(1.0, 1.0);
            if (on_line && line_near) {
                consumers[index].color = vec4(0, 1, 0, 0);
            //    let ball_mass = pow(src_ball.radius, 2.0) * PI;
            //    let other_mass = ball_mass;
            //    let other_velocity = -src_ball.velocity.xy;
            //    let c = 2.*dot(fake_n, (other_velocity - src_ball.velocity.xy)) / (dot(fake_n, fake_n) * (1./ball_mass + 1./other_mass));
            //    let new_vel = src_ball.velocity.xy + c/ball_mass * n;
            //    velocity = vec4(new_vel.x, new_vel.y, 0.0, 0.0);
            }
        }
