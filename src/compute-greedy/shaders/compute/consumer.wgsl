...
fn main(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
    ...
    if (at_destination) {
        ...
        if (at_home) {
            ...
            let max_demand_rate = min(c.balance, demand_rate);
            if (max_demand_rate > 0) {
              search_for_producer(index);
            }
            return;
        }
        ...
        let max_demand_rate = i32(min(max_consumer_can_buy, demand_rate));
        let purchase_price = u32(max_demand_rate) * producers[pid].params.price;

        let old_inventory = atomicSub(&producers[pid].inventory, max_demand_rate);
        if (old_inventory < max_demand_rate) {
          _ = atomicAdd(&producers[pid].inventory, max_demand_rate);
          return;
        }
        ...
        consumers[index].inventory += u32(max_demand_rate);
...
fn find_nearest_stocked_producer(c: Consumer, index: u32) -> i32 {
    ...
    for(var i: u32 = 0; i < stats.num_producers; i++){
        ...
        let max_consumer_can_buy = c.balance / producers[pid].params.price;
        let max_demand_rate = i32(min(max_consumer_can_buy, consumer_params.demand_rate));
        if (dist < shortest_distance && inventory > max_demand_rate) {
